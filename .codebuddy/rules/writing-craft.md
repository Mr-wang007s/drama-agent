...
| **A7** | **章节内数字/符号分节**（"一"/"二"/"（一）"/"1"/"其一"/"Part 1" 独立段）| **0** |
| **A8** | **场景分隔 `＊ ＊ ＊` / `***`** | **≤ 1 次/集**（仅限大跨度时空跳跃）|
| **A9** | **章节一级标题** `# 第{中文数字}章 · {标题}` | **必须存在**（全篇唯一 · 章名取自 beat-sheet 的 title）|
...
**A7 场景切换替代方案**（禁止数字分节 → 用什么替代？）：

| ❌ 禁止 | ✅ 正确做法 |
|---|---|
| 独立段"一/二/三" | 一个空行 + 场景首句的**时间/地点锚**（如"10:20。电话响。"、"安阳医院二楼那间办公室里"）|
| 独立段"1/2/3" | 同上 |
| 独立段"（一）/（1）" | 同上 |
| 独立段"其一/其二" | 同上 |
| "Part 1 / Scene 1 / Section A" | 同上 |

**A8 `＊ ＊ ＊` 使用原则**：
- 默认**不用**。绝大多数场景切换（同空间时间推进 / 换视角但时空连续）用一个空行即可
- 仅当**真正的大跨度时空跳跃**（如从北京酒店切到 2006 年殷墟闪回外的另一条独立线索）才考虑使用
- 全篇**至多 1 次**。第 2 次出现 = 滥用 · 门控 Warning
