...
- [ ] 秘密是否只在合理条件下才暴露？
- [ ] 内心独白是否体现 ghost → wound → lie → shield 链？
- [ ] 关系互动是否与 trust 值一致？
...
**交付 Phase 4 评审前必须跑自动化工具**：

```bash
npm run drama -- check-style --story <name> --episode <ep-id>
```

工具是 `drama-critic/scripts/check-ai-taste.js`，会打印量化报告并在有 Error 时 exit 1。
...
**Phase 4 评审时 Critic 会再跑一次，两层保护**。

### C. 文风原则
- **句式长短交错**：不要全碎片化短句，也不要全长句
- **段落有呼吸**：叙事段 3-6 句为主体，不是每段只有一句
- **少解释多留白**：不用旁白替角色解释动机
- **具体胜过抽象**：用感官细节代替抽象概念
- **拒绝对称美学**：不追求结构上的完美对仗
- **内心戏融入动作**：思想和行为交织，不分离
- **修辞克制**：一个场景最多 1-2 处比喻，不堆砌
