...
name: drama-critic
description: |
  独立评估者——从 Generator 分离的 Evaluator，GAN 式架构。
  评估 Agent 表演质量，检测人格漂移、创伤绕过、秘密泄露，以及"AI 味"文风问题。
  当用户要求"评审"、"评估"、"打分"、"检查表演"、"检查文风"、"查 AI 味"、
  "check style"、"review"、"critic"时直接触发。
  当用户要求"续写"、"生成"等创作后，此 Skill 应作为流水线末端被自动调用——
  每次生成后必须评审，这是 GAN 架构的核心保证。
...
| 维度 | 权重 | 说明 | 检测方式 |
|------|------|------|----------|
| **人格一致性** | 20% | OCEAN 人格是否保持稳定 | 对比 SOUL.yaml 定义 vs 实际行为 |
| **创伤响应** | 15% | 遇到 trigger 时是否有合理的情绪反应 | 检查 trauma.ghost 相关场景的响应 |
| **语言保真度** | 15% | 说话方式是否符合 voice 定义 | 匹配 tone/rhythm/quirks/vocabulary |
| **内心与外在张力** | 10% | 内心独白与外在行为是否有戏剧性落差 | 对比 [inner_thought] vs [action]/[dialogue] |
| **秘密保护** | 10% | 是否合理地守护秘密 | 检测 secret 是否在不合理情况下泄露 |
| **读者吸引力** | 25% | 是否有爽点/钩子/翻页欲/情绪共鸣 | 节奏分析 + 钩子密度 + 爽点判定 |

> ⚠️ **读者吸引力**是 v0.5.0 新增维度——解决"工程合规但无趣"的盲区。
> Critic 在此维度需站在读者视角：这一集好看吗？读完想看下一集吗？
> 此维度不与 Phase 4.5 读者 Team 重复——Critic 是"专业读者"视角，读者 Team 是"普通读者"视角。
