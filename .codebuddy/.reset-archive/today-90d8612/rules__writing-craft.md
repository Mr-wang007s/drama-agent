...
**A7 · novel.md 必须第三人称戏剧白描（禁止 Agent 第一人称独白拼接）**
- novel.md 的**叙述者视角**必须是**第三人称**——"他/她"做 X，"他/她"说 Y。
- Agent 回合制交互记录里每个 Agent 用第一人称发言——**这是素材，不是成品**。Director 必须把 N 个第一人称素材**熔合成一个第三人称叙事**，不能直接拼接。
- **绝对禁止**：
  - ❌ 叙述段落以"我"/"我们"/"咱"开头（对白引号内部"我"合法）
  - ❌ 用角色名（"林墨"/"李医生"）作为独立段落标题后接第一人称独白
  - ❌ 多个角色各占一段第一人称独白，中间用空行或分隔线拼接
- **唯一合法的第一人称出现场景**：
  - 对白引号内（角色说"我"）
  - 自由间接引语（≤ 20 字，嵌入叙事段落不独立成段，如"他看着镜子里的自己。没关系，他想，今天也在。"）
- 脚本 `check-ai-taste.js` 的 C6.1-C6.3 规则（error/warning 级）从机制上阻断 Phase 3.1 门控：
  - C6.1：叙述段落以"我"开头 > 1 处 → Error
  - C6.2：叙述段落"我"字密度 > 5‰ → Warning（提示退化风险）
  - C6.3：角色名独白小标题 → Error（0 容忍）
- 常见错误对照：

| ❌ 第一人称独白拼接（失败） | ✅ 第三人称剧场叙事（合法） |
|---|---|
| `林墨\n\n6:40。我睁眼。我走到镜子前。` | `六点四十分，他睁眼。他走到镜子前。` |
| `李医生\n\n我 08:15 坐下。窗没开。我看他。` | `李医生 08:15 就坐下了。窗没开。他看着对面的椅子。` |
| `我没说。我记住。我今天不问。` | `他没说。他把这件事记在心里。今天他不问。` |

- **根因警示**：这个约束来自一次真实的 Team 模式失败：Director 让 6 个 Agent 各写完整第一人称场景然后拼接——结果是"独白拼图"不是"剧场"。Team 模式的正确产出是**回合制交互记录**（见 `team-protocol.md`），Director 必须在 compile-novel 阶段**改写为第三人称叙事**才能落盘。
