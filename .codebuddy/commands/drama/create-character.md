...
```bash
# 交互式创建（回答 5 个核心问题）
drama-agent create-character --story fog-manor --interactive

# 从简介生成（AI 扩展）
drama-agent create-character --story fog-manor --from-brief "一个因童年火灾失去母亲的舞台监督，表面冷静但内心充满愧疚"

# 从原型启发
drama-agent create-character --story fog-manor --archetype "反英雄" --genre "悬疑"

# 从小说/剧本提取
drama-agent create-character --story fog-manor --extract-from novel.txt --character-name "林七"

# 指定角色 ID
drama-agent create-character --story fog-manor --id "my-character" --interactive
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `--story` | string | **推荐** | 故事名称（stories/<name>/） |
